...
const REFERRER_REWARD = 4; // Updated from 20
const REFERRED_REWARD = 2; // Updated from 5
...
async function fetchUserData() {
    ...
    try {
        ...
        if (userSnap.exists()) {
            ...
            // If user data somehow disappears, try to re-register them as a new user or guide to support
...
async function renderUI(userData) {
    if (!userData) {
        ...
        // Attempt to re-fetch if userData is unexpectedly null
...
async function renderEarningsSummary(userData) {
    ...
    querySnapshot.forEach(doc => {
        ...
        // User B's initial 2 points should also be counted for their own total earnings
...
async function renderReferralSection(userData) {
    // Update referral message for 4 AS / 2 AS
    const referralInfoText = document.querySelector('#referrals .info-text');
    if (referralInfoText) {
        referralInfoText.textContent = `Refer a friend and earn ${REFERRER_REWARD} AS points after they complete initial tasks. Your friend gets ${REFERRED_REWARD} AS points!`;
    }
...
async function renderWithdrawSection(userData) {
    ...
    // Update messages based on points
    if (userData.currentBalanceAS < BINANCE_WITHDRAW_POINTS) {
        ...
        binanceWithdrawMsg.textContent = `Ready to withdraw $${BINANCE_USD_VALUE}!`;
        binanceWithdrawOption.classList.add('active-glow');
    }

    if (userData.currentBalanceAS < GOOGLE_PLAY_WITHDRAW_POINTS) {
        ...
        googlePlayWithdrawMsg.textContent = `Ready to withdraw $${GOOGLE_PLAY_USD_VALUE}!`;
...
navItems.forEach(item => {
    item.addEventListener('click', async (e) => { // Made async
        ...
        try {
            ...
            if (userId) { // Only fetch user data if userId is available
                await fetchUserData(); // Always refresh user data after screen change
            } else {
                console.warn("Navigation attempted before user data loaded. Skipping fetchUserData.");
                showToast('App not fully loaded. Please wait.', 'warning', '⏳');
            }
        } catch (error) {
    ...
    try {
        ...
        if (userId) { // Only fetch user data if userId is available
            await fetchUserData(); // Ensure data is fresh for the displayed screen
             ...
             console.warn("Hashchange detected before user data loaded. Skipping fetchUserData.");
        }
    } catch (error) {
...
submitReferralCodeBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // Update current user (the referred one)
