...
let userPhotoUrl = null; // Telegram user photo is generally not directly accessible via WebApp initData for security.
                         // Often, developers use bot API to fetch it or a placeholder.
...
async function registerOrLoginUser(initData, userData) {
    try {
        // In a real app, this would be a Cloud Function call for security and initData validation.
        // For this client-side example, we'll directly interact with Firestore.
        // IMPORTANT: NEVER TRUST initData directly on client-side for critical operations.
        // Always send initData to your backend for validation and then process.
        ...
        if (!userSnap.exists()) {
            ...
            await setDoc(userRef, {
                ...
                profilePhotoUrl: userPhotoUrl, // Storing for UI convenience, can be fetched dynamically
            ...
            currentUserData = (await getDoc(userRef)).data(); // Fetch the newly created data
        } else {
            // Existing user login
...
async function fetchUserData() {
    ...
    try {
        ...
        if (userSnap.exists()) {
            ...
            console.error("User data not found after initial load.");
...
async function renderUI(userData) {
    ...
    renderCheckinStreak(userData.streakDay, userData.lastCheckinAt);
    renderEarningsSummary(userData);
    renderAdsMetrics(userData);
    renderTelegramTasks();
    renderReferralSection(userData);
    renderWithdrawSection(userData);
    renderProfileSection(userData);
...
function renderCheckinStreak(currentStreak, lastCheckinAt) {
    ...
    let displayStreak = isCheckedInToday ? currentStreak : currentStreak - 1; // If not checked in today, show previous streak.

    // If a day was missed, the streak should be visually reset to 0/1.
    // Backend logic handles the actual reset.
    if (lastCheckinDate && (today.getTime() - lastCheckinDate.getTime()) > (24 * 60 * 60 * 1000)) {
        ...
        checkinMessage.textContent = `You've checked in today! Come back tomorrow for Day ${displayStreak % 7 === 0 ? 1 : (displayStreak + 1)}`;
        claimCheckinBtn.textContent = `CHECKED IN`;
        claimCheckinBtn.disabled = true;
    } else {
        checkinMessage.textContent = `Claim your daily points! Day ${displayStreak % 7 === 0 ? 7 : (displayStreak % 7) + 1}`;
        claimCheckinBtn.textContent = `CLAIM DAY ${displayStreak % 7 === 0 ? 7 : (displayStreak % 7) + 1}`;
    ...
    for (let i = 1; i <= 7; i++) {
        ...
        // Current day to highlight (if not checked in today, it's the *next* day in the sequence)
        const dayToClaim = (displayStreak % 7) === 0 ? 7 : (displayStreak % 7) + 1;
        if (!isCheckedInToday && i === dayToClaim) {
...
async function renderEarningsSummary(userData) {
    // This requires aggregation of tasks for 'today' (UTC)
    const today = new Date();
    today.setUTCHours(0, 0, 0, 0);
    const todayTimestamp = today.getTime();

    // In a real app, this would be a Cloud Function that aggregates daily earnings
    // to prevent excessive client-side reads.
    const tasksRef = collection(db, 'tasks');
    const q = query(tasksRef,
        where('userId', '==', userId),
        where('createdAt', '>=', new Date(todayTimestamp))
    );
    ...
    querySnapshot.forEach(doc => {
        ...
        if (task.taskType === 'REFERRAL_CREDIT') referrals += task.rewardPoints;
    });

    todayAdsEarnings.textContent = ads;
    todayTgEarnings.textContent = tg;
    todayReferralEarnings.textContent = referrals;
...
async function renderAdsMetrics(userData) {
    ...
    // Lifetime requires querying tasks collection (or a pre-calculated field in user data)
    // For simplicity, we assume userData.totalTasksCompleted includes all tasks,
    // so we'd need to filter by taskType = AD, or have a separate field.
    // Let's assume `totalAdsCompleted` in userData for this display
...
async function renderReferralSection(userData) {
    ...
    if (userData.referredBy) {
        ...
        referralEntryMessage.style.color = var(--text-color-accent);
    } else {
...
navItems.forEach(item => {
    item.addEventListener('click', (e) => {
        ...
        // Re-render data for the active screen if needed
        fetchUserData(); // Always refresh user data
    });
});
...
window.addEventListener('hashchange', () => {
    ...
    navItems.forEach(nav => {
        if (nav.dataset.screen === currentHash) {
...
claimCheckinBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // 1. Verifying if user already checked in today (UTC)
        // 2. Calculating new streak and points (resets if day missed)
        // 3. Updating user's balance, streakDay, lastCheckinAt
        // 4. Logging the task
        ...
        renderCheckinStreak(newStreakDay, currentUserData.lastCheckinAt); // Re-render with new data
...
startAdTaskBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // The Monetag SDK `show_9725833()` call
        // IMPORTANT: The success callback below is client-side.
        // For real security, you'd need a server-side verification from Monetag (if available)
        // or implement a robust token-based client-server verification.
        // For this example, we assume client-side success is reported to our backend (Cloud Function).
        ...
        if (userSawAd) {
            ...
            // This function would handle:
            // 1. Rate limiting (hourly/daily)
            // 2. Updating user's balance and ad counts
            // 3. Logging the task
            // 4. Preventing double-crediting (idempotency keys)
            // Replace with actual Cloud Function call:
            ...
            renderAdsMetrics(currentUserData); // Re-render metrics
...
tgTaskList.addEventListener('click', async (event) => {
    ...
    try {
        ...
        // After user *returns* (or a slight delay), we'd usually trigger a verification.
        // For a real app, the user would come back, and click a 'Verify' button,
        // or a timer would trigger a server-side check.
        // For this example, we'll simulate a verification after a short delay.
        // A better approach would be to have a "Verify" button visible after they click "Join"
        // which calls a Cloud Function to check if they actually joined using the Bot API.

        // Simulating auto-verification after a delay (NOT SECURE FOR PRODUCTION)
        setTimeout(async () => {
            // --- Call Cloud Function for secure TG join verification ---
            // This function would use Telegram Bot API to check user's membership.
            // Replace with actual Cloud Function call:
            // const response = await callCloudFunction('verifyTelegramJoin', { userId, channelLink: link });

            // Client-side simulation of verification:
            // In a real scenario, the Cloud Function would respond with success/failure
...
submitReferralCodeBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // This function would handle:
        // 1. Validating the referrer code exists
        // 2. Checking if user already has a referrer
        // 3. Updating `referredBy` for the current user
        // 4. Incrementing referrer's `referralsCount` (initially, points paid later)
        // 5. Awarding initial REFERRED_REWARD (5 points) to the current user
        // Replace with actual Cloud Function call:
        ...
        animateBalance(asPointsBalanceDisplay, oldBalance, newBalance);
        renderReferralSection(currentUserData); // Re-render to disable input
...
async function checkInitialTasksCompletion() {
    ...
    // Check if required tasks are completed by the current user (the referred one)
    // Example: All 4 Telegram tasks + 15 Ad tasks
    ...
    if (tgTasksSnap.size >= requiredTelegramTasks && adTasksSnap.size >= requiredAds) {
        ...
        showToast('You completed your initial tasks!', 'success', '🎉');
...
async function checkReferralPayout(referrerId, referredId) {
    ...
    // It prevents referrer from getting points if referred user is fraudulent or not completing tasks
    ...
    if (referrerSnap.exists() && referredSnap.exists()) {
        ...
        if (referredUserData.initialTasksCompleted && !referrerUserData.flags.includes('referral_paid_for_' + referredId)) {
            ...
            if (userId === referrerId) {
                ...
                renderReferralSection(currentUserData);
            }
...
requestBinanceWithdrawBtn.addEventListener('click', async () => {
    ...
    // Basic format validation for Binance Pay ID (often starts with "B" and numbers)
    if (!/^B\d{8,16}$/.test(binanceId)) { // Example regex, adjust as needed
    ...
    try {
        ...
        // This function would handle:
        // 1. Validating balance
        // 2. Creating withdrawal record
        // 3. Deducting points from user balance (atomically)
        ...
        renderWithdrawSection(currentUserData); // Re-render to update history and button state
...
document.addEventListener('DOMContentLoaded', () => {
    if (tg.initDataUnsafe && tg.initDataUnsafe.user) {
        registerOrLoginUser(tg.initData, tg.initDataUnsafe.user);
    } else {
        ...
        registerOrLoginUser('dummy_init_data', dummyUser);
...
async function callCloudFunction(functionName, data) {
    ...
    const response = await fetch(`YOUR_CLOUD_FUNCTIONS_BASE_URL/${functionName}`, {
        method: 'POST',
        headers: {
