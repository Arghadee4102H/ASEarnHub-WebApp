...
async function registerOrLoginUser(initData, userData) {
    try {
        ...
        if (!userSnap.exists()) {
            ...
            currentUserData = newUserDoc; // Set currentUserData with the new document
...
function renderCheckinStreak(currentStreak, lastCheckinAt) {
    ...
    let displayStreak = isCheckedInToday ? currentStreak : currentStreak; // Show current streak if checked in, else it's the target day
...
historyTabBtns.forEach(btn => {
    btn.addEventListener('click', async (e) => { // Made async
        ...
        const historyType = e.target.dataset.history-type;
...
document.addEventListener('DOMContentLoaded', async () => { // Made async
    ...
    // Initial render call from here might be redundant if registerOrLoginUser already calls it,
    // but ensures the UI is consistent with the initial hash.
    // If you want to avoid double rendering, you could move the initial renderUI call from registerOrLoginUser here,
    // after the initialHash logic. For now, it will render twice on load, which is harmless.
    // await fetchUserData(); // This will be called by registerOrLoginUser and hashchange listener if applicable.
});
