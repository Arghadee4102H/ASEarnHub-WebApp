...
async function initializeApp() {
    ...
    // 1. Capture Telegram Init Data
    const initData = Telegram.WebApp.initData || ''; // For local testing, initDataUnsafe might be null
    ...
    if (initDataUnsafe && initDataUnsafe.user) {
        ...
        // Fallback for local testing or if user data is not available
        telegramUser = {
            id: 'test_user_12345', // Replace with a unique ID for local testing
            username: 'testuser',
            first_name: 'Test',
            last_name: 'User',
    ...
    // 2. Fetch or Create User in Firebase
    await fetchOrCreateUser(telegramUser);
    
    // 3. Load user-specific tasks and withdrawals
    await loadUserTasks();
    ...
    // 5. Hide loader and show main app
    setTimeout(() => {
...
document.getElementById('start-ads-task-btn').addEventListener('click', async () => {
    ...
    try {
        ...
        if (typeof show_9725833 === 'function') {
            ...
            console.error("Monetag SDK function 'show_9725833' not found.");
...
document.querySelectorAll('.join-tg-btn').forEach(button => {
    button.addEventListener('click', async (e) => {
        ...
        showToast("Redirecting to Telegram. Please join the channel/group.", "info", 5000);
...
Telegram.WebApp.onEvent('viewportChanged', () => {
    if (Telegram.WebApp.isVisible) {
        // Only run full init if current user data is not yet loaded, or for a refresh
        if (!currentUserData) {
            initializeApp();
        } else {
            // Otherwise, just ensure UI is up-to-date
            updateAllUI();
        }
    }
});
